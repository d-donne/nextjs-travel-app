import { PEOPLE_URL } from "@/constants";
import Image from "next/image";

interface CampProps {
  bgImage: string;
  title: string;
  subtitle: string;
  peopleJoined: number;
}
const CampSite = ({ bgImage, title, subtitle, peopleJoined }: CampProps) => {
  return (
    <div
      className={`h-full w-full min-w-[1100px] ${bgImage} bg-cover bg-no-repeat 2xl:rounded-5xl`}
    >
      <div className="flex flex-col h-full items-start justify-between p-6 lg:px-20 lg:py-10">
        <div className="flexCenter gap-4">
          <div className="rounded-full bg-green-50 p-4">
            <Image src="/folded-map.svg" alt="map" width={28} height={28} />
          </div>
          <div className="flex flex-col gap-1">
            <h4 className="bold-18 text-white">{title}</h4>
            <p className="regular-14 text-white">{subtitle}</p>
          </div>
        </div>
        <div className="flexCenter gap-6">
          <span className="flex -space-x-4 ">
            {PEOPLE_URL.map((url) => (
              <Image
                key={url}
                src={url}
                alt="person"
                width={40}
                height={40}
                className="inline-block h-10 w-10 rounded-full cursor-pointer"
              />
            ))}
          </span>
          <p className="bold-16 md:bold-20 text-white">
            {peopleJoined}+ joined
          </p>
        </div>
      </div>
    </div>
  );
};

const Camp = () => {
  return (
    <section className="relative 2xl:max-container flex flex-col py-10 lg:mb-10 lg:py-20 xl:mb-20">
      <div className="hide-scrollbar flex h-[340px] w-full items-start justify-start gap-8 overflow-x-auto lg:h-[400px] xl:h-[640px] ">
        <CampSite
          bgImage="bg-bg-img-1 rounded-r-5xl"
          title="Putuk Truno Camp"
          subtitle="Pridgen, Pasuruan"
          peopleJoined={50}
        />
        <CampSite
          bgImage="bg-bg-img-2 rounded-l-5xl"
          title="Mountain View Camp"
          subtitle="Somewhere, In the world"
          peopleJoined={37}
        />
      </div>
      <div className="flexEnd mt-10 px-6 lg:-mt-60 lg:mr-6">
        <div className="relative w-full bg-green-50 p-8 lg:max-w-[500px] xl:max-w-[734px] xl:rounded-5xl xl:px-16 xl:py-20 overflow-hidden rounded-3xl">
          <h2 className="regular-24 md:regular-32 2xl:regular-64 text-white">
            <strong className="">Feeling Lost</strong> and Not Knowing the Way?
          </h2>
          <p className="text-white regular-14 xl:regular-16 mt-5">
            Starting from the anxiety of the climbers when visiting a new
            climbing location, the possibility of getting lost is very high.
            That's why we are here for those of you who want to start an
            adventure.
          </p>
          <Image
            src="/quote.svg"
            alt="quote"
            width={186}
            height={219}
            className="camp-quote"
          />
        </div>
      </div>
    </section>
  );
};

export default Camp;
